...
use crate::parse::ascii::parse;

pub fn process_line(line: &[u8], nth: u32, mut sum: u64) -> u64 {
    let nth = nth - 1;
    ...
    sum = sum * 10 + (v - b'0') as u64;
    if nth == 0 {
        sum
    } else {
        process_line(&line[(idx + 1)..], nth, sum)
    }
}
...
pub fn run(content: &[u8]) -> Result<u64> {
    ...
    let result = lines
        ...
        .map(|line| {
            assert!(line.len() >= 12);
            process_line(line.as_slice(), 12, 0)
        })
        .sum();
