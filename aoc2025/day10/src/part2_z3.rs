use std::{io::Write, sync::atomic::AtomicUsize, thread::available_parallelism};
...
pub fn run(content: &[u8]) -> Result<u64, AdventError> {
    ...
    let percent = AtomicUsize::default();
    print!("{:>5.01}%", 0.0,);
