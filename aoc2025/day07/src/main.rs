...
enum Command {
    ...
    /// Part 2 Direct
    Part2Direct { file: Option<PathBuf> },

    /// Part 2 Chunk
    Part2Chunk { file: Option<PathBuf> },
}
...
fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Some(Command::Part2Direct { file }) => {
            println!("Result: {}", part2_direct::run(&read_input_u8!(file)?)?)
        }
        Some(Command::Part2Chunk { file }) => {
            println!("Result: {}", part2_chunk::run(&read_input_u8!(file)?)?)
        }
